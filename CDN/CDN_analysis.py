...
def CDN_multi_sub(folder_name, data_file, stimuli_folder, val_pair, dt, lam, mu=[0], lam_1=[0], N=50, fold=0.5, share_stimuli=True, max_iter=100, tol=1e-2, num_cores=1, B_u=True, C_u=True):
    ...
    for i in range(n1):
        ...
        for j in range(config.y.shape[0]):
            ...
            axarr[0].plot(t, estimated_x[j,(n1+1):(row_n-n1)], color='xkcd:purple', label='estimated', linewidth=2)
            #axarr[0].set_xlabel('Time (Sec)')
         
            axarr[1].plot(t, estimated_y[j,(n1+1):(row_n-n1)], color='xkcd:purple', label='estimated', linewidth=2)
            axarr[1].plot(t, y[j,(n1+1):(row_n-n1)], color='xkcd:blue', label='real', linewidth=2)
